(function() {
  var Field, Validations, Validator;
    ...
    Validations.validateRequired = function(element) {
      if (element.attr('required')) {
        if (element.attr('type') === 'radio') {
          ...
          if (/^\s*$/.test(element.val()) || !element.val()) {
            return element.addError('blank');
          }
        }
      }
    };
  ...
  Field = (function() {
    ...
    Field.messages = function(message) {
      this._messages || (this._messages = $('[data-merda]').data('merda'));
      if (message) {
  ...
  Validator = (function() {
    ...
    Validator.validate = function(form) {
      var validator;
      validator = new this(form);
      if (validator.isValid()) {
        return true;
      } else {
        this.manageErorrs(validator);
        return false;
      }
    };

    Validator.manageErorrs = function(validator) {
      return alert(validator.errorMessage());
    };
    ...
    Validator.prototype.errorMessage = function() {
